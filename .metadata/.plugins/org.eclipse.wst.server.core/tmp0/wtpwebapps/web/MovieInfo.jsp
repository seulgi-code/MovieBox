...
<head>
  ...
  <style type="text/css">
  em,strong.line_type a{
     color: lightgray;
  }
  input#insertR, input#submit{
...
.detail_top_wrap .detail_info2 .sub_info3 .line_type a {border-bottom:1px solid #666; color:;}
 ...
 */
#review_insert tr:hover {background-color: #ddd; color: black;}
...
#review tr:hover {background-color: #ddd; color: black;}
...
<script type="text/javascript">
   $(function(){   
      ...
      $("#insertR").click(function(){
         //alert($("#rating").val()+$("#id").val()+$("#con").val());
         $('#insertReview').submit();
         //$(this).find('rv').focus();
         //$('ul li').eq(2).focus();
      });
...
<div id="contents" class="contents_movie_detail" >
   ...
   <c:forEach var="i" items="eachInfo" varStatus="cnt"></c:forEach>
      <div class="poster_info">
         <img src="/orm/img/poster/${bean.poster_img}">
      </div>
      <div class="tit_info">
         <span class="ic_grade gr_15">15세이상관람가 </span> 
         <strong><em>${bean.movie_title}</em></strong>
      </div>
      ...
      <ul class="detail_info2">
         <li class="sub_info1">
            ...
            <strong>
               <em>${bean.genre }</em>
               <em> ${bean.opendate }</em>
               <em class="time_info">${bean.runningtime }분</em>
            </strong>
         </li>
         <li class="sub_info2">
            <em>감독</em>
            <strong class="line_type"><a href="#">${bean.director }</a></strong>
         </li>
         <li class="sub_info3">
            <em>출연</em>
            <strong class="line_type">
               <a href="#">${bean.actor }</a><!-- , <a href="#">엘르 패닝</a>, <a href="#">셀레나 고메즈</a>, <a href="#">주드 로</a>, <a href="#">디에고 루나</a> -->
            </strong>
         </li>
      </ul>
...
<div class="container">
  ...
  <ul class="nav nav-tabs">
    ...
    <li id="rv" name="rv"><a data-toggle="tab" href="#menu2">평점 및 관람평</a></li>
  </ul>
...
<div class="content" style="margin-top: 20px;">
  ...
  <form id="insertReview" name="insertReview" method="post" action="/orm/insertReview.do"> 
  <td></td>
    <td><%=sf.format(nowTime)%></td>
    <td><input type="text" id="id" name="id" placeholder="id" value="${id}"></td>
    <td><input type="text" id="con" name="con" placeholder="review" style="width: 515px;" /></td>
    <td id="star_grade">
       <a href="#" class="star1" id="1" lang="1"><i id="star1" >★</i></a>
       <a href="#" class="star2" id="2" lang="2">★</a>
       <a href="#" class="star" id="3" lang="3">★</a>
       <a href="#" class="star" id="4" lang="4">★</a>
       <a href="#" class="star" id="5" lang="5">★</a>
    </td>
    <td><input type="button" id="insertR" name="insertR" value="등록"> </td>
    <input type="hidden" value="" id="rating" name="rating">
    <input type="hidden" value="${mbean.movie_no}" id="movie_no" name="movie_no">
...
<form id="rvt" name="rvt" method="post" action="/orm/spreadReview.do">
<c:forEach var="i" items="${review}" varStatus="cnt">
  <tr id="content">
  <td id="${i.renum}"  name="nt" value="${i.renum}" ></td>
    <td>${i.regdate }</td>
    <td>${i.id}</td>
    <td>${i.con}</td>
    <td id="showrating">
       <c:if test="${i.rating == 1}">★</c:if>
       <c:if test="${i.rating == 2}">★★</c:if>
       <c:if test="${i.rating == 3}">★★★</c:if>
       <c:if test="${i.rating == 4}">★★★★</c:if>
       <c:if test="${i.rating == 5}">★★★★★</c:if>    
    </td>
